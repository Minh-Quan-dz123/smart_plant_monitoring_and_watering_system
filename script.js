...
async function getLatestSensorAPI(gardenId) {
    try {
        // Gọi API lấy dữ liệu sensor mới nhất từ DB
        ...
        if (!response.ok) {
            return null; // Nếu chưa có dữ liệu thì trả về null
        }
        return await response.json(); // Trả về { temperature, airHumidity, soilMoisture }
    } catch (error) {
...
// 2. UI: Hàm cập nhật giao diện cảm biến (Nhiệt độ, Độ ẩm...)
function updateSensorUI(data) {
    ...
    if (tempEl) {
        tempEl.textContent = (data.temperature !== undefined && data.temperature !== null) 
    ...
    if (humiEl) {
        humiEl.textContent = (data.airHumidity !== undefined && data.airHumidity !== null) 
            ? `${data.airHumidity}%` : "--%";
    }
    ...
    if (soilEl) {
        soilEl.textContent = (data.soilMoisture !== undefined && data.soilMoisture !== null) 
            ? `${data.soilMoisture}%` : "--%";
    }
}
